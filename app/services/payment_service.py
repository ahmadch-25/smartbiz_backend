...
def get_payments(db: Session) -> list[PaymentRecord]:
    statement = select(PaymentRecord).order_by(
...
def create_payment(
    ...
    if target_invoice_id is not None:
        invoice = db.get(Invoice, target_invoice_id)
        if invoice is None:
            raise InvoiceNotFoundError("Invoice not found")
    ...
    if invoice is not None:
        paid_amount = db.scalar(
            ...
            ).where(PaymentRecord.invoice_id == invoice.id)
        )
