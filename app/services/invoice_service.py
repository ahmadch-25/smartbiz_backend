...
def get_invoice_with_details(db: Session, invoice_id: int) -> Invoice | None:
    statement = (
        select(Invoice)
...
def create_invoice(db: Session, payload: InvoiceCreate) -> Invoice:
    ...
    return get_invoice_with_details(db, invoice.id) or invoice
...
def update_invoice(
    ...
    invoice = get_invoice_with_details(db, invoice_id)
    if invoice is None:
        return None
